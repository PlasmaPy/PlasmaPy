...
parallel build, where :samp:`{n}` is replaced with the number of processes or
``auto``. To skip building example notebooks, use ``make html-nonb`` instead.
...
modifying a CSS_ file. Using ``make clean-api`` instead will only remove the API
portion of the documentation build.
