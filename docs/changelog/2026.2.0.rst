...
- The ``mamba-org/setup-micromamba`` GitHub action is now used to install ``graphviz``
  and ``pandoc`` during documentation build workflows. Because this
...
- Fixed a typo in a formula in the
  `~plasmapy.formulary.frequencies.plasma_frequency` docstring. (:pr:`3216`)
- Fixed a typo in :file:`two_fluid_dispersion.ipynb`. (:pr:`3218`)
- Fixed a typo in the docstring of `~plasmapy.formulary.laser.em_wavelength`.
  (:pr:`3219`)
...
- Corrected `~plasmapy.formulary.frequencies.plasma_frequency_lite` to
  take the absolute value of the charge number ``Z``. Because the
  absolute value of ``Z`` had not previously been taken, the plasma
  frequency was able to be negative. This bug did not occur in
  `~plasmapy.formulary.frequencies.plasma_frequency`. (:pr:`3216`)
...
- Switched the pytest configuration in :file:`pyproject.toml` to use the
  ...
  legacy INI compatibility mode. (:pr:`3147`)
...
- Added continuous integration workflows to check `PHEP 3
  <https://github.com/heliophysicsPy/standards/blob/e3a1391834726f615620603b2847708e3ef5cf3a/pheps/phep-0003.md>`__
  compliance and compatibility with the Python in Heliophysics Community
  environment. (:pr:`3211`)
