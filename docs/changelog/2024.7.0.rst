...
- Renamed the ``binding_energy`` attribute of |Particle| to
...
- Added a |Nox| session that invokes ``autotyping`` to automatically
  add type annotations, using either the ``--safe`` or
...
- Applied type annotations using ``autotyping``, and made other updates
  to type annotations and docstrings. (:pr:`2728`)
- Added type annotations to `plasmapy.utils.roman`. (:pr:`2733`)
- Added type annotations to ``plasmapy.utils._units_helpers``.
  (:pr:`2734`)
- Added a |Nox| session for building the changelog. (:pr:`2744`)
- Added an experimental |Nox| session for adding type annotations using
