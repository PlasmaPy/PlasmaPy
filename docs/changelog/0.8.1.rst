...
- Made numerous minor updates and fixes to |reST| links in docstrings
...
- Increased the strictness of the ``build_docs`` |tox| environment so
  that broken |reST| links now emit warnings which are then treated as
...
- Bumped the minimum versions of |mpmath| to ``1.2.1``, `numpy` to
  ...
  <https://github.com/plasmapy/plasmapy/pull/1482>`__)
- Moved |h5py|, |lmfit|_, |mpmath|, and |numba| out of the
...
- Added ``dlint``, ``flake8``, ``flake8-absolute-import``,
  ...
  ``pygments`` into the ``tests`` requirements category and pre-commit_
