...
class NullPoint(Point):
    ...
    def isEqual(self, point):
        return np.isclose(self.getLoc()[0], point.getLoc()[0]) and np.isclose(self.getLoc()[1],
                                                                              point.getLoc()[1]) and np.isclose(
            self.getLoc()[2], point.getLoc()[2])
...
def vector_space(func, x_range, y_range, z_range, precision=[0.01, 0.01, 0.01]):
    ...
    x, y, z = np.meshgrid(np.linspace(x_range[0], x_range[1], x_den),
                          np.linspace(y_range[0], y_range[1], y_den),
                          np.linspace(z_range[0], z_range[1], z_den), indexing='ij')
    u, v, w = func(x, y, z)
...
def trilinear_approx(vspace, cell):
    ...
    def approx_func(xInput, yInput, zInput):
        Bx = ax + bx * xInput + cx * yInput + dx * zInput + ex * xInput * yInput + fx * xInput * zInput + gx * yInput * zInput + hx * xInput * yInput * zInput
        By = ay + by * xInput + cy * yInput + dy * zInput + ey * xInput * yInput + fy * xInput * zInput + gy * yInput * zInput + hy * xInput * yInput * zInput
        Bz = az + bz * xInput + cz * yInput + dz * zInput + ez * xInput * yInput + fz * xInput * zInput + gz * yInput * zInput + hz * xInput * yInput * zInput
...
def jacobian(vspace, cell):
    ...
    def jacobian_func(xInput, yInput, zInput):
        ...
        jmatrix = np.array(
            [float(dBxdx), float(dBxdy), float(dBxdz),
             float(dBydx), float(dBydy), float(dBydz),
             float(dBzdx), float(dBzdy), float(dBzdz)]).reshape(3, 3)
        return jmatrix
...
def reduction(vspace, cell):
    ...
    for point in corners:
        if u[point[0]][point[1]][point[2]] == 0 or np.sign(u[point[0]][point[1]][point[2]]) != sign:
            passX = True

    # Check Y-Component
    sign = np.sign(v[cell[0], cell[1], cell[2]])
    for point in corners:
        if v[point[0]][point[1]][point[2]] == 0 or np.sign(v[point[0]][point[1]][point[2]]) != sign:
            passY = True

    # Check Z-Component
    sign = np.sign(w[cell[0], cell[1], cell[2]])
    for point in corners:
        if w[point[0]][point[1]][point[2]] == 0 or np.sign(w[point[0]][point[1]][point[2]]) != sign:
            passZ = True
...
def trillinear_analysis(vspace, cell):
    ...
    coeffs = [[ax, bx, cx, dx, ex, fx, gx, hx],
              [ay, by, cy, dy, ey, fy, gy, hy],
              [az, bz, cz, dz, ez, fz, gz, hz]]
    normalized_coeffs = []
    for elem in coeffs:
    ...
    initial = np.array([vspace[0][0][f000[0]][f000[1]][f000[2]], vspace[0][1][f000[0]][f000[1]][f000[2]],
    ...
    # Front Surface
    yConst1 = (vspace[0][1][f000[0]][f000[1]][f000[2]])  # y-coordinate of the front surface
    # Bx=By=0 Curve Endpoint

    root1, root2 = bilinear_root(ax + cx * yConst1, bx + ex * yConst1, dx + gx * yConst1, fx + hx * yConst1,
                                 ay + cy * yConst1, by + ey * yConst1, dy + gy * yConst1, fy + hy * yConst1)
    ...
    root1, root2 = bilinear_root(ax + cx * yConst1, bx + ex * yConst1, dx + gx * yConst1, fx + hx * yConst1,
                                 az + cz * yConst1, bz + ez * yConst1, dz + gz * yConst1, fz + hz * yConst1)
    if root1 != None:
        if is_close(root1, root2):
    ...
    root1, root2 = bilinear_root(ay + cy * yConst1, by + ey * yConst1, dy + gy * yConst1, fy + hy * yConst1,
                                 az + cz * yConst1, bz + ez * yConst1, dz + gz * yConst1, fz + hz * yConst1)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Back Surface
    yConst2 = (vspace[0][1][f111[0]][f111[1]][f111[2]])  # y-coordinate of the front surface
    # Bx=By=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + cx * yConst2, bx + ex * yConst2, dx + gx * yConst2, fx + hx * yConst2,
                                 ay + cy * yConst2, by + ey * yConst2, dy + gy * yConst2, fy + hy * yConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=BZ=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + cx * yConst2, bx + ex * yConst2, dx + gx * yConst2, fx + hx * yConst2,
                                 az + cz * yConst2, bz + ez * yConst2, dz + gz * yConst2, fz + hz * yConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # By=Bz=0 Curve Endpoint
    root1, root2 = bilinear_root(ay + cy * yConst2, by + ey * yConst2, dy + gy * yConst2, fy + hy * yConst2,
                                 az + cz * yConst2, bz + ez * yConst2, dz + gz * yConst2, fz + hz * yConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=By=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + bx * xConst1, cx + ex * xConst1, dx + fx * xConst1, gx + hx * xConst1,
                                 ay + by * xConst1, cy + ey * xConst1, dy + fy * xConst1, gy + hy * xConst1)
    ...
    # Bx=BZ=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + bx * xConst1, cx + ex * xConst1, dx + fx * xConst1, gx + hx * xConst1,
                                 az + bz * xConst1, cz + ez * xConst1, dz + fz * xConst1, gz + hz * xConst1)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # By=Bz=0 Curve Endpoint
    root1, root2 = bilinear_root(ay + by * xConst1, cy + ey * xConst1, dy + fy * xConst1, gy + hy * xConst1,
                                 az + bz * xConst1, cz + ez * xConst1, dz + fz * xConst1, gz + hz * xConst1)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=By=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + bx * xConst2, cx + ex * xConst2, dx + fx * xConst2, gx + hx * xConst2,
                                 ay + by * xConst2, cy + ey * xConst2, dy + fy * xConst2, gy + hy * xConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=BZ=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + bx * xConst2, cx + ex * xConst2, dx + fx * xConst2, gx + hx * xConst2,
                                 az + bz * xConst2, cz + ez * xConst2, dz + fz * xConst2, gz + hz * xConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # By=Bz=0 Curve Endpoint
    root1, root2 = bilinear_root(ay + by * xConst2, cy + ey * xConst2, dy + fy * xConst2, gy + hy * xConst2,
                                 az + bz * xConst2, cz + ez * xConst2, dz + fz * xConst2, gz + hz * xConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=By=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + dx * zConst1, bx + fx * zConst1, cx + gx * zConst1, ex + hx * zConst1,
                                 ay + dy * zConst1, by + fy * zConst1, cy + gy * zConst1, ey + hy * zConst1)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=BZ=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + dx * zConst1, bx + fx * zConst1, cx + gx * zConst1, ex + hx * zConst1,
                                 az + dz * zConst1, bz + fz * zConst1, cz + gz * zConst1, ez + hz * zConst1)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # By=Bz=0 Curve Endpoint
    root1, root2 = bilinear_root(ay + dy * zConst1, by + fy * zConst1, cy + gy * zConst1, ey + hy * zConst1,
                                 az + dz * zConst1, bz + fz * zConst1, cz + gz * zConst1, ez + hz * zConst1)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=By=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + dx * zConst2, bx + fx * zConst2, cx + gx * zConst2, ex + hx * zConst2,
                                 ay + dy * zConst2, by + fy * zConst2, cy + gy * zConst2, ey + hy * zConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # Bx=BZ=0 Curve Endpoint
    root1, root2 = bilinear_root(ax + dx * zConst2, bx + fx * zConst2, cx + gx * zConst2, ex + hx * zConst2,
                                 az + dz * zConst2, bz + fz * zConst2, cz + gz * zConst2, ez + hz * zConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    # By=Bz=0 Curve Endpoint
    root1, root2 = bilinear_root(ay + dy * zConst2, by + fy * zConst2, cy + gy * zConst2, ey + hy * zConst2,
                                 az + dz * zConst2, bz + fz * zConst2, cz + gz * zConst2, ez + hz * zConst2)
    if root1 != None:
        if is_close(root1, root2):
    ...
    if np.sign(tlApprox(BxByEndpoints[0][0], BxByEndpoints[0][1], BxByEndpoints[0][2])[2]) * \
            np.sign(tlApprox(BxByEndpoints[1][0], BxByEndpoints[1][1], BxByEndpoints[1][2])[2]) > 0:
        isNullPoint = False

    # Checking sign of third component for Bx=Bz=0
    if np.sign(tlApprox(BxBzEndpoints[0][0], BxBzEndpoints[0][1], BxBzEndpoints[0][2])[1]) * \
            np.sign(tlApprox(BxBzEndpoints[1][0], BxBzEndpoints[1][1], BxBzEndpoints[1][2])[1]) > 0:
        isNullPoint = False

    # Checking sign of third component for By=Bz=0
    if np.sign(tlApprox(ByBzEndpoints[0][0], ByBzEndpoints[0][1], ByBzEndpoints[0][2])[0]) * \
            np.sign(tlApprox(ByBzEndpoints[1][0], ByBzEndpoints[1][1], ByBzEndpoints[1][2])[0]) > 0:
        isNullPoint = False
...
def locate_null_point(vspace, cell, n, err):
    ...
    pos_000 = np.array([vspace[0][0][f000[0]][f000[1]][f000[2]], vspace[0][1][f000[0]][f000[1]][f000[2]],
                        vspace[0][2][f000[0]][f000[1]][f000[2]]])
    pos_111 = np.array([vspace[0][0][f111[0]][f111[1]][f111[2]], vspace[0][1][f111[0]][f111[1]][f111[2]],
    ...
    # Adding the Corners
    for point in corners:
        starting_pos.append([vspace[0][0][point[0]][point[1]][point[2]], vspace[0][1][point[0]][point[1]][point[2]],
                             vspace[0][2][point[0]][point[1]][point[2]]])
    # Adding the Mid Point
    starting_pos.append(
        [vspace[0][0][f000[0]][f000[1]][f000[2]] + deltax / 2.0, vspace[0][1][f000[0]][f000[1]][f000[2]] + deltay / 2.0,
    ...
    for x0 in starting_pos:
        ...
        for i in range(n):
            ...
            x0 = np.subtract(x0, np.matmul(np.linalg.inv(jcb(x0[0], x0[1], x0[2])), Bx0))
    ...
    if divide > MAX_DIVIDE:
        ...
        return None
    new_vspace = vector_space(tlApprox, [pos_000[0], pos_111[0]], [pos_000[1], pos_111[1]], [pos_000[2], pos_111[2]],
...
def vector_Space_Func(x, y, z):
    a = -1.80 - 2.57 * x + 6.92 * y + 0.44 * z + 0.02 * y * z + 0.46 * x * z - 1.40 * x * y
    b = 0.44 - 3.05 * x + 2.09 * y + 0.20 * z - 0.46 * y * z - 0.34 * x * z + 1.46 * x * y
    c = -0.67 - 2.30 * x + 8.69 * y + 0.48 * z + 1.40 * y * z - 1.46 * x * z - 8.29 * x * y
    return [a,b,c]
