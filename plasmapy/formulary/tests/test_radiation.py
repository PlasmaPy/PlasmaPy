...
def test_thermal_bremsstrahlung():
    ...
    frequencies = (10**np.arange(15, 16, .01))/u.s
    ne, Te = 1e22*u.cm**-3, 1e2*u.eV
    ion_species = 'H+'

    spectrum = thermal_bremsstrahlung(frequencies, ne, Te,
    ...
    small_frequencies = (10**np.arange(12, 16, .01))/u.s
    with pytest.raises(PhysicsError):
        spectrum = thermal_bremsstrahlung(small_frequencies, ne, Te,
                                          ion_species=ion_species)

    # Test violates Rayleigh-Jeans limit
    small_Te = 1* u.eV
    with pytest.raises(PhysicsError):
        spectrum = thermal_bremsstrahlung(frequencies, ne, small_Te,
                                          ion_species=ion_species)
