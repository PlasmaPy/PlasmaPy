...
class MagnetoStatics(abc.ABC):
    ...
    def _magnetic_field(self, p: u.m) -> np.ndarray:
        """
...
class CircularWire(Wire):
    ...
    def __init__(self, normal, center: u.m, radius: u.m,
        ...
        @numba.njit
        def _magnetic_field(p, pt, dl, current) -> u.T:
            r"""
            Calculate magnetic field generated by this wire at position `p`
            ...
            r_norm_3 = np.sum(r**2, axis=0)**(3/2)

            ft = np.cross(dl.T, r.T)/np.expand_dims(r_norm_3, 1)  # (n, 3)

            return np.pi*(np.expand_dims(w, 0) @ ft)[0, ...] \
                * scipy.constants.mu_0/4/np.pi*current
