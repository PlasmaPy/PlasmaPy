...
class MagneticDipole(MagnetoStatics):
    ...
    def magnetic_field(self, p: u.m) -> u.T:
        ...
        Notes
        -----
        The magnetic field :math:`\vec{B}` at a position defined by
        the displacement vector :math:`\vec{r}` from a magnetic dipole
        with a magnetic dipole moment :math:`\vec{m}` can be expressed as
...
class GeneralWire(Wire):
    ...
    def magnetic_field(self, p: u.m, n: numbers.Integral = 1000) -> u.T:
        ...
        current :math:`I` can be found at a point in 3D space using the
        Biot–Savart law.

        Let the position be represented by the position vector :math:`\vec{p}`.
        If the wire follows the curve described by the parametric vector
        equation :math:`\vec{l}(t)`, the line integral of the Biot–Savart law
        can be approximated by segmenting the wire into :math:`n` straight
        pieces of equal length. The :math:`i\text{th}` wire element can then be
        written as :math:`\Delta\vec{\ell}_i = \vec{l}(t_i) - \vec{l}(t_{i - 1})`
        where :math:`t_i = t_{\min} + i(t_{\max}-t_{\min})/n`. Furthermore,
        ...
        The magnetic field :math:`\vec{B}` at position :math:`\vec{p}` can then
        be approximated as

        .. math::
            \vec B \approx \frac{\mu_0 I}{4\pi}
            \sum_{i=1}^{n}\frac{\vec{\Delta\ell}_i \times \vec{r}_i}{\left| \vec{r}_i \right|^3}
...
class FiniteStraightWire(Wire):
    ...
    def magnetic_field(self, p) -> u.T:
        ...
        constant electric current :math:`I` can be found at a point
        in 3D space using the Biot–Savart law.

        Let the point where the magnetic field will be calculated be
        represented by the point :math:`r_0` and the wire's beginning
        and end as :math:`r_1` and :math:`r_2`, respectively. These
        points correspond to position vectors :math:`\vec{r}_0`,
        :math:`\vec{r}_1`, and :math:`\vec{r}_2` (or ``p``, ``p1``,
        and ``p2``), respectively. Further, the vector from points
        :math:`r_i` to :math:`r_j` can be written as
        :math:`\vec{r}_{ij} = \vec{r}_j - \vec{r}_i`.

        Next, consider the triangle with the points :math:`r_0`,
        :math:`r_1`, and :math:`r_2` as vertices. The foot of
        perpendicular opposite the vertex :math:`r_0`, which will be
        used to find the unit vector in the direction of the magnetic
        field, can be expressed as

        .. math::
            \vec{r}_f = \vec{r}_1 + \vec{r}_{12}
            \frac{\vec{r}_{10} \cdot \vec{r}_{12}}
            {|\vec{r}_{12}|^2}

        The magnetic field :math:`\vec{B}` generated by the wire found at
        the point :math:`r_0` can be found using the Biot–Savart law which
        ...
        :math:`\vec{r}_{10}` (:math:`\vec{r}_{20}`) and
        :math:`\vec{r}_{12}` with
        ...
        .. math::
            \hat{B} = \frac{\vec{r}_{12} \times r_{f0}}
            {|\vec{r}_{12} \times r_{f0}|}

        is the unit vector in the direction of the magnetic field
        at the point :math:`r_0`.
        """
...
class InfiniteStraightWire(Wire):
    ...
    def magnetic_field(self, p) -> u.T:
        ...
        length and constant electric current can be found at a point
        in 3D space using the Biot–Savart law.
        ...
        wire with constant current :math:`I` at point :math:`p` can
        ...
        is the perpendicular distance between the wire and point :math:`p`,
        and
