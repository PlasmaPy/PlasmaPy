...
from plasmapy.transport import fundamental_electron_collision_freq, fundamental_ion_collision_freq
...
class ClassicalTransport:
    ...
    def __init__(
        ...
        if not is_valid_field:
            raise ValueError(f"Unknown field orientation " f"'{self.field_orientation}'")
        ...
        if m_i is None:
            try:
                ...
                raise ValueError(
                    f"Unable to find mass of particle: " f"{ion_particle} in ClassicalTransport"
                )
        else:
        ...
        if coulomb_log_ei is not None:
            ...
            self.coulomb_log_ei = Coulomb_logarithm(
                T_e, n_e, (self.e_particle, self.ion_particle), V_ei, method=coulomb_log_method
            )
    ...
    def resistivity(self) -> u.Ohm * u.m:
        ...
        tau_e = 1 / fundamental_electron_collision_freq(
            self.T_e, self.n_e, self.ion_particle, self.coulomb_log_ei, self.V_ei
        )
    ...
    def ion_thermal_conductivity(self) -> u.W / u.m / u.K:
        ...
        tau_i = 1 / fundamental_ion_collision_freq(
            self.T_i, self.n_i, self.ion_particle, self.coulomb_log_ii, self.V_ii
        )
    ...
    def electron_viscosity(self) -> u.Pa * u.s:
        ...
        if eta1[0].unit == eta1[2].unit and eta1[2].unit == eta1[4].unit:
            ...
            eta = (
                np.array(
                    (eta1[0].value, eta1[1].value, eta1[2].value, eta1[3].value, eta1[4].value)
                )
                * unit_val
            ).to(u.Pa * u.s)
        return eta
...
def _nondim_thermal_conductivity(hall, Z, particle, model, field_orientation, mu=None, theta=None):
    ...
    if _is_electron(particle):
        if model == "spitzer-harm" or model == "spitzer":
            ...
            raise ValueError(f"Unrecognized model '{model}' in " "_nondim_thermal_conductivity")
    else:
        if model == "braginskii":
...
def _nondim_viscosity(hall, Z, particle, model, field_orientation, mu=None, theta=None):
    ...
    if _is_electron(particle):
        ...
        if model == "braginskii":
            ...
            raise NotImplementedError("Ion viscosity is not " "implemented in the Spitzer model.")
        else:
            raise ValueError(f"Unrecognized model '{model}' in " "_nondim_viscosity")
    return eta_hat
...
def _nondim_resist_ji_held(hall, Z, field_orientation):
    ...
    def f_alpha_perp(Z_idx):
        ...
        denominator = (
            r ** (5 / 3) + alpha_2[Z_idx] * r ** (4 / 3) + alpha_1[Z_idx] * r + alpha_0[Z_idx]
        )
        return 1 - numerator / denominator
...
def _nondim_visc_e_ji_held(hall, Z):
    ...
    def f_eta_2(Z_idx, r):
        numerator = (6 / 5 * Z + 3 / 5 * np.sqrt(2)) * r + hprime_0[Z_idx] * eta_0_e[Z_idx]
        denominator = (
            r ** 3
...
def _nondim_tc_i_ji_held(hall, Z, mu, theta, field_orientation, K=3):
    ...
    if K == 3:
        ...
        Delta_perp_i1 = (
            r ** 4 + (1.352 + 12.49 * zeta + 34 * zeta ** 2) * r ** 2 + 0.1693 * Delta_par_i1 ** 2
        )
        kappa_perp_i = (
    ...
    if field_orientation == "all":
        return np.array(
            (kappa_par_i / np.sqrt(2), kappa_perp_i / np.sqrt(2), kappa_cross_i / np.sqrt(2))
        )
    ...
    if K == 3:
        ...
        def Delta_perp_i2(r, zeta, Delta_par_i2):
            Delta_perp_i2 = (
                ...
                + (3.191 + 49.62 * zeta + 306.4 * zeta ** 2 + 808.1 * zeta ** 3 + 784 * zeta ** 4)
                * r ** 2
        ...
        def f_eta_2(r, zeta, Delta_perp_i2):
            eta_2_i = (
                (3 / 5 * np.sqrt(2) + 2 * zeta) * r ** 2 + 0.5820 * eta_0_i * Delta_par_i2 ** 2
            ) / Delta_perp_i2
            return eta_2_i
