...
from plasmapy.physics.exceptions import PhysicsError, PhysicsWarning, RelativityWarning
...
from plasmapy.transport.braginskii import (
...
from plasmapy.transport import ClassicalTransport
...
class Test_classical_transport:
    ...
    def test_spitzer_vs_formulary(self):
        ...
        with pytest.warns(RelativityWarning):
            ...
            testTrue = np.isclose(ct2.resistivity().value, alpha_spitzer_perp_NRL.value, rtol=2e-2)
            errStr = (
    ...
    def test_resistivity_units(self):
        ...
        errStr = (
            f"Resistivity units should be {u.Ohm * u.m} and " f"not {self.ct.resistivity().unit}."
        )
        assert testTrue, errStr
    ...
    def test_coulomb_log_calc(self):
        ...
        with pytest.warns(RelativityWarning):
            ...
            cl_ii = Coulomb_logarithm(
                self.T_i, self.n_e, [self.ion_particle, self.ion_particle], self.V_ii
            )
            cl_ei = Coulomb_logarithm(self.T_e, self.n_e, ["e", self.ion_particle], self.V_ei)
            testTrue = cl_ii == ct2.coulomb_log_ii
            errStr = (
                f"Ion-ion coulomb logarithm should be {cl_ii} " f"and not {ct2.coulomb_log_ii}."
            )
        assert testTrue, errStr
        testTrue = cl_ei == ct2.coulomb_log_ei
        errStr = (
            f"Electron-ion coulomb logarithm should be {cl_ei} " f"and not {ct2.coulomb_log_ei}."
        )
        assert testTrue, errStr
    ...
    @pytest.mark.parametrize(
        "model, method, field_orientation, expected",
        [
    ...
    def test_number_of_returns(self, model, method, field_orientation, expected):
        with pytest.warns(RelativityWarning):
            ct2 = ClassicalTransport(
                T_e=self.T_e,
                n_e=self.n_e,
            ...
            errStr = (
                f"{method} in {model} model returns "
    ...
    @pytest.mark.parametrize(
        "model, expected",
        [
            ("ji-held", np.array([0.07674402, 0.07674402, 0.07674402, 0, 0]) * u.Pa * u.s),
            ("braginskii", np.array([0.07674402, 0.07671874, 0.07671874, 0, 0]) * u.Pa * u.s),
        ],
    )
    def test_electron_viscosity_by_model(self, model, expected):
        with pytest.warns(RelativityWarning):
            ...
            testTrue = np.allclose(ct2.electron_viscosity(), expected, atol=1e-6 * u.Pa * u.s)
            errStr = (
    ...
    @pytest.mark.parametrize(
        "model, expected",
        [
            ("ji-held", np.array([7.96226452, 7.96226452, 7.96226452, 0, 0]) * u.Pa * u.s),
            ("braginskii", np.array([7.91936173, 7.89528642, 7.89528642, 0, 0]) * u.Pa * u.s),
        ],
    )
    ...
    def test_electron_thermal_conductivity_by_model(self, model, expected):
        with pytest.warns(RelativityWarning):
            ...
            testTrue = np.allclose(
                ct2.electron_thermal_conductivity(), expected, atol=1e-6 * u.W / (u.K * u.m)
            )
            errStr = (
    ...
    @pytest.mark.parametrize(
        "key, expected",
        {
            ...
            "thermoelectric conductivity": [7.11083999e-01, 1.61011272e-09, 2.66496639e-05],
            "electron thermal conductivity": [4.91374931e06, 2.28808496e-03, 6.90324259e01],
            "electron viscosity": [
                7.51661800e-02,
                5.23617668e-21,
                2.09447067e-20,
    ...
    def test_dictionary(self, key, expected):
        ...
        errStr = f"Expected values of {key} are {expected} and not" f"{calculated.si.value}."
        assert testTrue, errStr
...
def test_nondim_thermal_conductivity_unrecognized_model(particle):
    with pytest.raises(ValueError):
        _nondim_thermal_conductivity(1, 1, particle, "standard model is best model", "parallel")
...
class Test__nondim_visc_i_braginskii:
    ...
    @pytest.mark.parametrize(
        "expected, power", [(np.array((0.96, 0.3, 1.2, 0.5, 1.0)), np.array((0, 2, 2, 1, 1)))]
    )
