...
coeff_table = np.load(os.path.join(data_dir, "braginskii_polynomial_fit_coefficients.npz"))
...
class BraginskiiPolynomialFit(AbstractPolynomialCoefficients):
    ...
    def _Delta(self, i):
        return self.chi_e**4 + self._c['delta1'][i]*self.chi_e**2 + self._c['delta0'][i]
    ...
    def _norm_alpha_perp(self):
        i = self._find_nearest_Z(self.Z)
        return 1 - (self._c["alpha1p"][i] * self.chi_e**2 + self._c['alpha0p'][i])/ self._Delta(i)
                    

    @property
    def _norm_alpha_cross(self):
        i = self._find_nearest_Z(self.Z)
        return self.chi_e*(self._c['alpha1pp'][i]*self.chi_e**2 + self._c['alpha0pp'][i])/ self._Delta(i)
