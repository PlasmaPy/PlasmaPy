...
def test_two_fluid_dispersion_solution():
    ...
    sol = two_fluid_dispersion_solution(
        B=B, k=k, n=n, T_e=T_e, T_i=T_i, theta=theta, z=z
    )

    sol_theta_0 = two_fluid_dispersion_solution(
        B=B, k=k, n=n, T_e=T_e, T_i=T_i, theta=0 * u.deg, z=z
    )

    sol_theta_90 = two_fluid_dispersion_solution(
        B=B, k=k, n=n, T_e=T_e, T_i=T_i, theta=90 * u.deg, z=z
    )

    sol_B_0 = two_fluid_dispersion_solution(
        B=0 * u.T, k=k, n=n, T_e=T_e, T_i=T_i, theta=theta, z=z
    )

    sol_k_0 = two_fluid_dispersion_solution(
        B=B, k=0 * u.m ** -1, n=n, T_e=T_e, T_i=T_i, theta=theta, z=z
    )
    ...
    assert np.isclose(np.real(sol_theta_90["acoustic_mode"].value[0, 0]), 0.0007709854, rtol=1e-6)
    ...
    with pytest.raises(ValueError):
        two_fluid_dispersion_solution(
            B=B_neg, k=k, n=n, T_e=T_e, T_i=T_i, theta=theta, z=z
        )

    with pytest.raises(ValueError):
        two_fluid_dispersion_solution(
            B=B, k=k, n=n_neg, T_e=T_e, T_i=T_i, theta=theta, z=z
        )

    with pytest.raises(ValueError):
        two_fluid_dispersion_solution(
            B=B, k=k, n=n, T_e=T_e_neg, T_i=T_i, theta=theta, z=z
        )
    ...
    sol_k_arr = two_fluid_dispersion_solution(
        B=B, k=k_arr, n=n, T_e=T_e, T_i=T_i, theta=theta, z=z
    )
    sol_theta_arr = two_fluid_dispersion_solution(
        B=B, k=k, n=n, T_e=T_e, T_i=T_i, theta=theta_arr, z=z
    )
    sol_k_arr0 = two_fluid_dispersion_solution(
        B=B, k=k_arr[0], n=n, T_e=T_e, T_i=T_i, theta=theta, z=z
    )
    sol_theta_arr0 = two_fluid_dispersion_solution(
        B=B, k=k, n=n, T_e=T_e, T_i=T_i, theta=theta_arr[0], z=z
    )
    sol_k_arr1 = two_fluid_dispersion_solution(
        B=B, k=k_arr[-1], n=n, T_e=T_e, T_i=T_i, theta=theta, z=z
    )
    sol_theta_arr1 = two_fluid_dispersion_solution(
        B=B, k=k, n=n, T_e=T_e, T_i=T_i, theta=theta_arr[-1], z=z
    )
