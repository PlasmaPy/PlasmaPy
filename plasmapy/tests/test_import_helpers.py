...
def _create_distinct_versions(current_version, time):
    ...
    patch = extra[0] if len(extra) == 1 else 0
    ...
    different_versions = [
        f"{major}.{minor + increment}",
        f"{major + increment}.{minor}",
        ]

    if time == 'after' or patch > 0:
        different_versions.append(f"{major}.{minor}.{patch + increment}")

    if time == 'before':
        different_versions.append(current_version)
        different_versions.append(f"{major}.{minor}.0")

    return different_versions


@pytest.mark.parametrize('newer_version', _create_distinct_versions(numpy.__version__, 'newer'))
