...
Let's analyze various Langmuir probe characteristics using the
...
[bias, current] = pickle.load(open('langmuir_samples\\Beckers2017.p', 'rb'))

# Create the Characteristic object, taking into account the correct units
characterstic = Characteristic(np.array(bias) * u.V,
                               np.array(current)*1e3 * u.mA)

# Calculate the cylindrical probe surface area
probe_l = 1.145 * u.mm
probe_d = 1.57 * u.mm
probe_area = probe_l * np.pi * probe_d + np.pi * 0.25 * probe_d**2
...
print(swept_probe_analysis(characterstic, probe_area, 4,
                           visualize=True, plot_EEDF=True))
...
[bias, current] = pickle.load(open('langmuir_samples\\Pace2015.p', 'rb'))
characterstic = Characteristic(np.array(bias) * u.V,
                               np.array(current) * 1e3 * u.mA)

# Initially the electrons are assumed to be Maxwellian. To check this the fit
# of the electron growth region will be plotted.
swept_probe_analysis(characterstic, 0.738 * u.cm**2, 40,
                     bimaxwellian=False, plot_electron_fit=True)
...
print(swept_probe_analysis(characterstic, 0.738 * u.cm**2, 40,
                           bimaxwellian=True, visualize=True,
...
# This is reflected in the results, which indicates that the temperatures of
...
[bias, current] = pickle.load(open('langmuir_samples\\Beckers2017b.p', 'rb'))
characterstic = Characteristic(np.array(bias) * u.V,
                               np.array(current) * 1e3 * u.mA)
probe_l = 1.145 * u.mm
probe_d = 1.57 * u.mm
...
print(swept_probe_analysis(characterstic, probe_area, 4, bimaxwellian=True,
                           plot_electron_fit=True, visualize=True))
