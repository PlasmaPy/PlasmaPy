...
class Test_PlasmaBlob:
    ...
    def test_invalid_particle(self):
        ...
        with pytest.raises(InvalidParticleError):
            plasmablob.PlasmaBlob(T_e=self.T_e, n_e=self.n_e, Z=self.Z, particle="cupcakes")
    ...
    def test_coupling(self):
        ...
        errStr = (
            f"Coupling parameter should be {self.couplingVal} " f"and not {methodVal.si.value}."
        )
        testTrue = np.isclose(methodVal.value, self.couplingVal, rtol=1e-8, atol=0.0)
        assert testTrue, errStr
    ...
    def test_quantum_theta(self):
        ...
        errStr = (
            f"Degeneracy parameter should be {self.thetaVal} " f"and not {methodVal.si.value}."
        )
        testTrue = np.isclose(methodVal.value, self.thetaVal, rtol=1e-8, atol=0.0)
        assert testTrue, errStr
