...
def test_Alfven_speed():
    ...
    assert np.isclose(Alfven_speed(1 * u.T, 1e-8 * u.kg * u.m ** -3).value, 8920620.580763856)
...
def test_ion_sound_speed():
    ...
    assert np.isclose(
        ion_sound_speed(T_i=1.3232 * u.MK, T_e=1.831 * u.MK, ion="p", gamma_e=1, gamma_i=3).value,
        218816.06086407552,
    )

    assert np.isclose(
        ion_sound_speed(T_i=0.88 * u.MK, T_e=1.28 * u.MK, ion="p", gamma_e=1.2, gamma_i=3.4).value,
        193328.52857788358,
    )
    ...
    with pytest.warns(u.UnitsWarning):
        assert ion_sound_speed(T_e=1.2e6, T_i=0 * u.K) == ion_sound_speed(
            T_e=1.2e6 * u.K, T_i=0 * u.K
        )

    with pytest.warns(u.UnitsWarning):
        assert ion_sound_speed(T_i=1.3e6, T_e=0 * u.K) == ion_sound_speed(
            T_i=1.3e6 * u.K, T_e=0 * u.K
        )
...
def test_thermal_speed():
    ...
    assert np.isclose(thermal_speed(1 * u.MK, particle="p").si.value, 128486.56960876315)
    ...
    assert np.isclose(thermal_speed(1e6 * u.K, method="mean_magnitude").si.value, 6212510.3969422)

    assert np.isclose(thermal_speed(1e6 * u.K, method="rms").si.value, 6743070.475775486)
...
class Test_kappa_thermal_speed(object):
    ...
    def test_invalid_method(self):
        ...
        """
        with pytest.raises(ValueError):
            kappa_thermal_speed(self.T_e, self.kappa, particle=self.particle, method="invalid")
        return
    ...
    def test_probable1(self):
        ...
        errStr = (
            f"Kappa thermal velocity should be {self.probable1True} " f"and not {known1.si.value}."
        )
    ...
    def test_rms1(self):
        ...
        Tests if expected value is returned for a set of regular inputs.
        """
        known1 = kappa_thermal_speed(self.T_e, self.kappa, particle=self.particle, method="rms")
        errStr = f"Kappa thermal velocity should be {self.rms1True} " f"and not {known1.si.value}."
    ...
    def test_mean1(self):
        ...
        errStr = (
            f"Kappa thermal velocity should be {self.mean1True} " f"and not {known1.si.value}."
        )
...
def test_plasma_frequency():
    ...
    assert np.isclose(plasma_frequency(mu * u.cm ** -3, particle="p").value, 1.32e3, rtol=1e-2)
...
def test_inertial_length():
    ...
    assert np.isclose(inertial_length(mu * u.cm ** -3, particle="p").cgs.value, 2.28e7, rtol=0.01)
