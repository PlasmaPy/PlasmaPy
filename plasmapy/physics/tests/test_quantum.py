...
def test_thermal_deBroglie_wavelength():
    ...
    assert np.isclose(lambda_dbTh.value, lambda_dbTh_true, rtol=1e-15, atol=0.0), expectStr
...
def test_Fermi_energy():
    ...
    # test a simple case for expected value
    expectStr = "Fermi energy at 1e23 cm^-3 should be " f"{energy_F_true} and not {energy_F}."
    assert np.isclose(energy_F.value, energy_F_true, rtol=1e-15, atol=0.0), expectStr
    # testing returned units
...
def test_Thomas_Fermi_length():
    ...
    # test a simple case for expected value
    expectStr = (
        "Thomas-Fermi length at 1e23 cm^-3 should be " f"{lambda_TF_true} and not {lambda_TF}."
    )
    assert np.isclose(lambda_TF.value, lambda_TF_true, rtol=1e-15, atol=0.0), expectStr
    # testing returned units
...
def test_Wigner_Seitz_radius():
    ...
    testTrue = np.isclose(radiusMeth, radiusTrue, rtol=0.0, atol=1e-15 * u.m)
    errStr = f"Error in Wigner_Seitz_radius(), got {radiusMeth}, " f"should be {radiusTrue}"
...
class Test_chemical_potential:
    ...
    def test_known1(self):
        ...
        errStr = f"Chemical potential value should be {self.True1} and not " f"{methodVal}."
        assert testTrue, errStr
