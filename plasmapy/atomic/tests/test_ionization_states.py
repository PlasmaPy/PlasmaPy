...
class TestIonizationStatesItemAssignment:
    ...
    def test_setitem(self, element, new_states):
        ...
        try:
            ...
            pytest.fail("Unable to change ionic fractions for an " "IonizationStates instance.")
...
class TestIonizationStatesDensities:
    ...
    def test_number_densities(self, elem):
        assert np.allclose(
            self.states.number_densities[elem].value, self.expected_densities[elem].value
        ), (
...
class TestIonizationStatesAttributes:
    ...
    def test_kappa_defaults_to_inf(self):
        assert np.isinf(self.instance.kappa), "kappa does not default to a value of inf."

    @pytest.mark.parametrize("uninitialized_attribute", ["number_densities", "ionic_fractions"])
    ...
    def test_setting_abundances(self):
        ...
        log_new_abundances = {
            element: np.log10(new_abundances[element]) for element in new_abundances.keys()
        }
...
class TestIonizationStatesDensityEqualities:
    ...
    def setup_class(cls):
        ...
        cls.instances = {
            key: IonizationStates(**cls.dict_of_kwargs[key]) for key in cls.dict_of_kwargs.keys()
        }
    ...
    def test_number_densities_defined(self, test_key):
        ...
        for base_particle in self.instances[test_key].base_particles:
            assert not np.any(np.isnan(number_densities[base_particle])), (
                f"Test {test_key} should have number densities " f"defined, but doesn't."
            )
    ...
    def test_number_densities_undefined(self, test_key):
        ...
        for base_particle in self.instances[test_key].base_particles:
            assert np.all(np.isnan(number_densities[base_particle])), (
                f"Test {test_key} should not have number densities " f"defined, but does."
            )

    @pytest.mark.parametrize(
        "this, that",
        itertools.product(["ndens1", "ndens2", "no_ndens3", "no_ndens4", "no_ndens5"], repeat=2),
    )
    def test_equality(self, this, that):
        ...
        if expect_equality != are_equal:
            ...
            pytest.fail(f"Cases {this} and {that} should be {descriptor} but " f"are not.")
