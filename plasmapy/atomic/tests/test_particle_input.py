...
from plasmapy.atomic.exceptions import InvalidElementError
...
def func_simple_noparens(
    ...
    if not isinstance(particle, Particle):
        raise TypeError(f"The argument particle in func_simple_noparens is not a Particle")
    return particle
...
def func_simple_parens(
    ...
    """
    if not isinstance(particle, Particle):
        raise TypeError(f"The argument particle in func_simple_parens is not a Particle")
    return particle
...
decorator_categories_table = [
    ...
    (
        {"require": ["fermion", "charged"], "any_of": ["lepton", "baryon"], "exclude": ["matter"]},
        "p+",
        AtomicError,
    ),
]
