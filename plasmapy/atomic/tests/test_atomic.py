...
from ..symbols import atomic_symbol, isotope_symbol, element_name
...
equivalent_particle_mass_args = [
    ...
    ["T+", {}, "T 1+", {}, None],
    ["Tritium+", {}, "T", {"Z": 1}, None],
    ["Fe-56 1+", {}, "Fe", {"mass_numb": 56, "Z": 1}, particle_mass("Fe-56 1-") - 2 * const.m_e],
    ["Fe-56 +1", {}, 26, {"mass_numb": 56, "Z": 1}, None],
]


@pytest.mark.parametrize("arg1, kwargs1, arg2, kwargs2, expected", equivalent_particle_mass_args)
...
def test_half_life_unstable_isotopes():
    ...
    for isotope in _Isotopes.keys():
        if "half_life" not in _Isotopes[isotope].keys() and not _Isotopes[isotope].keys():
