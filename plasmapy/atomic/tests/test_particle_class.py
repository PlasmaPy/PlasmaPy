...
from plasmapy.utils import call_string, run_test_equivalent_calls
...
test_Particle_table = [
    ...
    (
        Particle("C"),
        {"Z": 3, "mass_numb": 14},
        {"particle": "C-14 3+", "element": "C", "isotope": "C-14", "ionic_symbol": "C-14 3+"},
    ),
]
...
def opposite(particle):
    try:
        ...
        raise InvalidParticleError(
            f"The unary ~ (invert) operator is unable to find the " f"antiparticle of {particle}."
        ) from exc
...
class Test_antiparticle_properties_inversion:
    ...
    def test_inverted_inversion(self, particle):
        ...
        assert particle == ~~particle, (
            f"~~{repr(particle)} equals {repr(~~particle)} instead of " f"{repr(particle)}."
        )
...
def test_particleing_a_particle(arg):
    ...
    assert particle == Particle(Particle(Particle(particle))), (
        f"Particle({repr(arg)}) does not equal " f"Particle(Particle(Particle({repr(arg)}))."
    )
