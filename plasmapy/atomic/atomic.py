...
def known_isotopes(argument: Union[str, Integral] = None) -> List[str]:
    ...
    def known_isotopes_for_element(argument):
        ...
        sorted_isotopes = [
            mass_number for (isotope, mass_number) in sorted(zip(mass_numbers, isotopes))
        ]
    ...
    if argument is not None:
        try:
            ...
            raise InvalidElementError(
                "known_isotopes is unable to get " f"isotopes from an input of: {argument}"
            )
...
def common_isotopes(
    ...
    if argument is not None:
        ...
        try:
            ...
            raise InvalidElementError(
                "common_isotopes is unable to get isotopes " f"from an input of: {argument}"
            )
...
def stable_isotopes(argument: Union[str, Integral] = None, unstable: bool = False) -> List[str]:
    """
    ...
    def stable_isotopes_for_element(
        ...
        StableIsotopes = [
            isotope for isotope in KnownIsotopes if _Isotopes[isotope]["stable"] == stable_only
        ]
    ...
    if argument is not None:
        try:
            ...
            raise InvalidElementError(
                "stable_isotopes is unable to get isotopes " f"from an input of: {argument}"
            )
    elif argument is None:
        isotopes_list = []
        for atomic_numb in range(1, 119):
