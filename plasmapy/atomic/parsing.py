...
def _dealias_particle_aliases(alias: Union[str, Integral]) -> str:
    ...
    if not isinstance(alias, str):
        symbol = alias
    elif alias in _case_sensitive_aliases.values() or alias in _case_insensitive_aliases.values():
        symbol = alias
...
def _parse_and_check_atomic_input(
    ...
    def _get_element(element_info: str) -> str:
        ...
        if element_info.lower() in _element_names_to_symbols.keys():
            ...
            raise InvalidParticleError(
                f"The string '{element_info}' does not correspond to " f"a valid element."
            )
    ...
    def _reconstruct_isotope_symbol(element: str, mass_numb: Integral) -> str:
        ...
        if mass_numb is not None:
            ...
            if isotope not in _Isotopes.keys():
                raise InvalidParticleError(
                    f"The string '{isotope}' does not correspond to " f"a valid isotope."
                )
    ...
    if Z is not None and Z_from_arg is not None:
        if Z != Z_from_arg:
            ...
            warnings.warn(
                "Redundant charge information for particle " f"'{argument}' with Z = {Z}.",
                AtomicWarning,
            )
...
def _call_string(arg: Union[str, Integral], kwargs: Dict = {}) -> str:
    """
