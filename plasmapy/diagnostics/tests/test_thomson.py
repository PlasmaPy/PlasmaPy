...
def width_at_value(x, y, val):
    """
    Calculates the width of a curve at a value.
    """
...
def test_collective_spectrum():
    ...
    assert np.isclose(alpha, 1.27, atol=.01), "check collective case alpha"
    ...
    assert np.isclose(i_width, 0.14, 1e-3), 'check collective case ion feature width'
    assert np.isclose(e_width, 16.32, 1e-3), 'check collective case electron feature width'
...
def test_non_collective_spectrum():
    ...
    #Check that alpha is correct
    assert np.isclose(alpha, 0.0403, atol=.01), "check non-collective case alpha"


    e_width = width_at_value(wavelength.value, Skw.value, .2e-13)

    #Check that the widts of the electron feature matchs expectations
    assert np.isclose(e_width, 28.68, 1e-3), 'check non-collective case electron feature width'
