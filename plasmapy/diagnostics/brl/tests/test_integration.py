...
class Test__integrate:
    ...
    @pytest.mark.parametrize(
        ("start_index", "end_index", "complex_before_start", "complex_after_end"), []
    )
    def test_complex_start_and_end_combinations(
        self, start_index, end_index, complex_before_start, complex_after_end
    ):
        ...
        if complex_before_start and complex_after_end:
            ...
            def integral_func(x):
                return 2 / 3 * ((x - start_index) ** 1.5 - (end_index - x) ** 1.5)
        ...
        assert np.isclose(
            ...
            (integral_func(end) - integral_func(start)),
        )
