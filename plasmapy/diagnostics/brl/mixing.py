...
def test_if_divergent_oscillations_exist(all_found_currents):
    r"""
    ...
    derivative_ratio_sum = np.sum(derivative_magnitudes[1:] / derivative_magnitudes[:-1])

    return derivative_ratio_sum > 6.3


def estimate_num_iterations(all_found_currents, relative_accuracy):
    r"""
    ...
    num_iterations = np.log(np.abs(a / (relative_accuracy * c))) / b

    return True, num_iterations, c


def estimate_num_iterations_oscillating(all_found_currents, relative_accuracy, estimated_final_current):
    r"""
    Estimate how many iterations are needed to get within the relative accuracy.
...
def determine_convergence(all_found_currents, relative_accuracy, estimated_final_current, new_net_charge_density, old_net_charge_density, x, zero_temperature_repelled_particles=False):
    r"""
    ...
    if zero_temperature_repelled_particles:
        ...
        if abs(2 * (max_current - min_current) / (max_current + min_current)) < relative_accuracy:
            return True
    else:
        ...
        # Line 140, 144.
        for index in (-1, -2):
            if abs(all_found_currents[index] - estimated_final_current) - abs(relative_accuracy * estimated_final_current):
                converged = False
                break

        # This is `Y` on line 266.
        squared_relative_error = (2 * (new_net_charge_density - old_net_charge_density) / (new_net_charge_density + old_net_charge_density))**2

        r = 1 / x
        # Integrate using the trapezoidal rule to find `TOT` in the code on line 267.
        integrated_error = (squared_relative_error[:-1] + squared_relative_error[1:]) * (r[1:] - r[:-1]) / 2
        # `AVGE` in code.
...
def determine_coefficients(
    ...
    """
    if control_level == 1:
        return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change, coefficients_previously_decreased

    # In the code, the `adjust_period` is 10.
    adjust_period = 10
    if all_found_currents.size % adjust_period != 0:
        return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change, coefficients_previously_decreased
    
    divergent_oscillations_exist = test_if_divergent_oscillations_exist(all_found_currents)
    ...
    if divergent_oscillations_exist:
        ...
        new_coefficient_1, new_coefficient_2 = decrease_mixing_coefficients(coefficient_1, coefficient_2)
        return new_coefficient_1, new_coefficient_2, False, 0, True
    
    # Line 123.
    if not divergent_oscillations_exist and control_level < 3:
        logging.info("Not checking for convergence rate since `control_level` is less than 3 and oscillations are not diverging.")
        return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change + 1, coefficients_previously_decreased

    # Line 132.
    if zero_temperature_repelled_particles:
        converged = determine_convergence(all_found_currents, relative_accuracy, 0, new_net_charge_density, old_net_charge_density, x, zero_temperature_repelled_particles)
        if not converged:
            ...
            return coefficient_1, coefficient_2, True, num_checks_since_coefficient_change + 1, coefficients_previously_decreased
    else:
        # Line 280.
        if num_checks_since_coefficient_change < 2:
            logging.info("Not checking for slow oscillation damping since `num_checks_since_coefficient_change` is less than 2.")
            return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change + 1, coefficients_previously_decreased

        converging, num_iterations, estimated_final_current = estimate_num_iterations(all_found_currents, relative_accuracy)
        if not converging:
            logging.info("Current to probe is not converging.")
            return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change + 1, coefficients_previously_decreased
        elif num_iterations > 40:
            # Lines 141 and 28.
            if control_level < 4 or coefficients_previously_decreased:
                logging.info(f"Convergence is too slow (iterations remaining = {num_iterations:.0f}). Not increasing coefficients because either the `control_level` is not high enough or the coefficients were previously decreased.")
                return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change + 1, coefficients_previously_decreased
            else:
                logging.info(f"Convergence is too slow (if coefficient left unchanged, iterations remaining = {num_iterations:.0f}). Increasing mixing coefficients.")
                new_coefficient_1, new_coefficient_2 = increase_mixing_coefficients(coefficient_1, coefficient_2)
                return new_coefficient_1, new_coefficient_2, False, 0, True
        ...
        has_converged = determine_convergence(
            ...
            old_net_charge_density,
            x,
            zero_temperature_repelled_particles
        )
        ...
        if has_converged:
            ...
            logging.info(f"Not checking for slow oscillation damping since `control_level` is less than 4 ({control_level = }). Expected number of iterations remaining = {num_iterations:.0f}.")
            return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change + 1, coefficients_previously_decreased
        ...
        if is_oscillating and not converges_fast:
            logging.info(f"Oscillations are damping too slowly (iterations remaining = {num_iterations:.0f}). Decreasing mixing coefficients.")
            new_coefficient_1, new_coefficient_2 = decrease_mixing_coefficients(coefficient_1, coefficient_2)
            return new_coefficient_1, new_coefficient_2, False, 0, True
        else:
            logging.info(f"Oscillations are damping quickly and convergence is in approximately {num_iterations:.0f} iterations.")
            return coefficient_1, coefficient_2, False, num_checks_since_coefficient_change + 1, coefficients_previously_decreased
