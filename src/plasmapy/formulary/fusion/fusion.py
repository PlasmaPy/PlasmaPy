...
reactivities from the literature. Higher-level functions for reactor
performance (Lawson criterion, ignition condition, fusion gain) are
also included.
...
_DT_IGNITION_TRIPLE_PRODUCT = 3.0e21  # keV s / m^3, D-T at T ~ 30 keV


@validate_quantities(
    n={"units": u.m**-3, "can_be_negative": False},
    T={"units": u.keV, "can_be_negative": False},
    tau_E={"units": u.s, "can_be_negative": False, "can_be_zero": False},
)
def lawson_product(
    n: u.Quantity[u.m**-3],
    T: u.Quantity[u.keV],
    tau_E: u.Quantity[u.s],
) -> u.Quantity[u.keV * u.s / u.m**3]:
    r"""Return the Lawson triple product nT\tau_E.

    The triple product nT\tau_E is the key figure of merit for
    magnetically confined fusion plasmas. Ignition requires this
    product to exceed a reaction-dependent threshold.

    Parameters
    ----------
    n : `~astropy.units.Quantity`
        Total fuel ion density in units convertible to m\ :sup:`-3`.
        For two-species reactions (e.g. D-T), a 50:50 mix is assumed.

    T : `~astropy.units.Quantity`
        Ion temperature in units convertible to keV.

    tau_E : `~astropy.units.Quantity`
        Energy confinement time in units convertible to s.

    Returns
    -------
    product : `~astropy.units.Quantity`
        Lawson triple product nT\tau_E in units of keV\ s / m\ :sup:`3`.

    Notes
    -----
    The D-T ignition threshold is approximately
    :math:`3 \times 10^{21}` keV\ s / m\ :sup:`3` at
    T \approx 30 keV.
...
def ignition_condition(
    reaction: str | tuple[str, str],
    n: u.Quantity[u.m**-3],
    T: u.Quantity[u.keV],
    tau_E: u.Quantity[u.s],
) -> u.Quantity[u.dimensionless_unscaled]:
    r"""Return the ratio of the Lawson triple product to the ignition threshold.

    The ratio :math:`\mathrm{nT\tau_E} / \mathrm{nT\tau_E,threshold}`
    indicates how close a plasma is to ignition. A ratio >= 1 means
    ignition is possible.

    The threshold is derived from power balance assuming
    α-particle heating alone:

    .. math::

        \frac{n^2}{4} \langle \sigma v \rangle E_\alpha \ge \frac{3nT}{\tau_E}

    giving :math:`\mathrm{nT\tau_E} \ge 12T^2 / (\langle \sigma v \rangle E_\alpha)`.
    ...
    n : `~astropy.units.Quantity`
        Total fuel ion density in units convertible to m\ :sup:`-3`.
        For two-species reactions, a 50:50 mix is assumed.
    ...
    ratio : `~astropy.units.Quantity`
        nT\tau_E divided by the reaction-specific ignition threshold.
        Dimensionless. >= 1 indicates ignition.

    Notes
    -----
    For D-T, the ignition threshold is approximately
    :math:`3 \times 10^{21}` keV\ s / m\ :sup:`3` at T \approx 30 keV,
    corresponding to a ratio of 1.

    Examples
    --------
    >>> import astropy.units as u
    >>> n, T, tau = 1e20 * u.m**-3, 30 * u.keV, 1 * u.s
    >>> ratio = ignition_condition("T(d,n)4He", n, T, tau)
    """
    params = _lookup_reaction(reaction)
    sv = reactivity(reaction, T)
    T_val = T.to(u.keV)
    E_alpha = params.Q_keV * params.fraction_charged * u.keV
    threshold = 12 * T_val**2 / (sv * E_alpha)
    product = lawson_product(n, T, tau_E)
    return (product / threshold).to(u.dimensionless_unscaled)
...
def fusion_gain(
    ...
    r"""Return the plasma fusion gain factor Q.

    The fusion gain :math:`Q = P_\mathrm{fus} / P_\mathrm{loss}`
    is the ratio of fusion power output to thermal power loss.
    Q = 1 is scientific break-even, Q = \infty is ignition.
    ...
    Q : `~astropy.units.Quantity`
        Plasma fusion gain factor. Dimensionless.

    Notes
    -----
    JET's record pulse achieved Q \approx 0.64. ITER is designed for
    Q = 10. Ignition corresponds to Q \rightarrow \infty (where
    α-particle heating balances all losses).

    Examples
    --------
    >>> import astropy.units as u
    >>> fusion_gain("T(d,n)4He", 30 * u.keV, 1e20 * u.m**-3, 1 * u.s)
    <Quantity ...>
    """
    params = _lookup_reaction(reaction)
    if params.identical_particles:
        n1 = n2 = n
    else:
        n1 = n2 = n / 2
    P_fus = fusion_power_density(reaction, T, n1, n2)
    P_loss = (3 * n * T / tau_E).to(u.W / u.m**3)
    return (P_fus / P_loss).decompose()
