...
def thermal_bremsstrahlung(
    ...
    <Quantity 8.175602...e-23 kg / (m s2)>
    >>> thermal_bremsstrahlung(
    ...     10 ** np.arange(15, 16, 0.1) * u.Hz, 1e22 * u.cm**-3, 1e2 * u.eV
    ... )
    <Quantity [ 79.59052..., 117.73282..., 127.85119..., 127.1250... ,
               121.01549..., 112.02367..., 101.45553...,  90.04503...,
                78.23475...,  66.32227...] kg / (m s2)>
    >>> thermal_bremsstrahlung(
    ...     1e17 * u.Hz, 1e16 * u.cm**-3, 1e4 * u.eV, ion="Fe-56 12+"
    ... )
    <Quantity 2.169328...e-10 kg / (m s2)>
    """
