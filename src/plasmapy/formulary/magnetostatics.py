...
class MagnetoStatics(abc.ABC):
    ...
    @abc.abstractmethod
    def magnetic_field(self, p: u.Quantity[u.m]) -> u.Quantity[u.T]:  # ty: ignore[not-subscriptable]
        """
...
class MagneticDipole(MagnetoStatics):
    ...
    @validate_quantities
    def __init__(self, moment: u.Quantity[u.A * u.m**2], p0: u.Quantity[u.m]) -> None:  # ty: ignore[not-subscriptable]
    ...
    def magnetic_field(self, p: u.Quantity[u.m]) -> u.Quantity[u.T]:  # ty: ignore[not-subscriptable]
        ...
        .. math::
            \vec{B} = \frac{}{4π}
            \left( \frac{3(\vec{m} · \vec{r})\vec{r}}{|\vec{r}|^5}
            - \frac{\vec{m}}{|\vec{r}|^3} \right)

        where :math:`μ_0` is vacuum permeability.
        """
        r = p - self.p0
        m = self.moment
        B = (
...
class GeneralWire(Wire):
    ...
    @validate_quantities
    def __init__(self, parametric_eq, t1, t2, current: u.Quantity[u.A]) -> None:  # ty: ignore[not-subscriptable]
    ...
    def magnetic_field(self, p: u.Quantity[u.m], n: int = 1000) -> u.Quantity[u.T]:  # ty: ignore[not-subscriptable]
        r"""
        Calculate magnetic field generated by this wire at position ``p``.
        ...
        :math:`t_{\min} ≤ t ≤ t_{\max}`. Further, let the
        ...
        .. math::
            \vec B = \frac{μ_0 I}{4π}
            \int_C \frac{d\vec{l} × \vec{r}}{|\vec{r}|^3}

        where :math:`μ_0` is the permeability of free space.
...
class FiniteStraightWire(Wire):
    ...
    def __init__(
        self,
        p1: u.Quantity[u.m],  # ty: ignore[not-subscriptable]
        p2: u.Quantity[u.m],  # ty: ignore[not-subscriptable]
        current: u.Quantity[u.A],  # ty: ignore[not-subscriptable]
    ) -> None:
        self.p1 = p1.value
        self.p2 = p2.value
        self._p1_u = p1.unit
    ...
    def magnetic_field(self, p) -> u.Quantity[u.T]:  # ty: ignore[not-subscriptable]
        r"""
        Calculate magnetic field generated by this wire at position ``p``.
        ...
        .. math::
            \vec{B} = \frac{μ_0 I}{4π} (\cos θ_1 - \cos θ_2)
            \hat{B}

        where :math:`μ_0` is the permeability of free space,
        :math:`θ_1` (:math:`θ_2`) is the angle between
        :math:`\vec{p}_{10}` (:math:`\vec{p}_{20}`) and
        :math:`\vec{p}_{12}` with

        .. math::
            \cos\theta_1 = \frac{\vec{p}_{10} · \vec{p}_{12}}
            {|\vec{p}_{10}| |\vec{p}_{12}|}, \quad
            \cos{θ_2} = \frac{\vec{p}_{20} · \vec{p}_{12}}
            {|\vec{p}_{20}| |\vec{p}_{12}|},

        and

        .. math::
            \hat{B} = \frac{\vec{p}_{12} × \vec{p}_{f0}}
            {|\vec{p}_{12} × \vec{p}_{f0}|}
...
class InfiniteStraightWire(Wire):
    ...
    def __init__(
        self,
        direction,
        p0: u.Quantity[u.m],  # ty: ignore[not-subscriptable]
        current: u.Quantity[u.A],  # ty: ignore[not-subscriptable]
    ) -> None:
    ...
    def magnetic_field(self, p) -> u.Quantity[u.T]:  # ty: ignore[not-subscriptable]
        ...
        .. math::
            \vec{B} = \frac{μ_0 I}{2π |\vec{r}|} \hat{B}

        where :math:`μ_0` is the permeability of free space,
        :math:`|\vec{r}| = |\vec{l} × (\vec{p} - \vec{p}_0)|`
        is the perpendicular distance between the wire and the point
        :math:`p`, and

        .. math::
            \hat{B} = \frac{\vec{l} × (\vec{p} - \vec{p}_0)}
            {|\vec{l} × (\vec{p} - \vec{p}_0)|}
...
class CircularWire(Wire):
    ...
    def __init__(
        self,
        normal,
        center: u.Quantity[u.m],  # ty: ignore[not-subscriptable]
        radius: u.Quantity[u.m],  # ty: ignore[not-subscriptable]
        current: u.Quantity[u.A],  # ty: ignore[not-subscriptable]
        n: int = 300,
    ) -> None:
    ...
    def magnetic_field(self, p) -> u.Quantity[u.T]:  # ty: ignore[not-subscriptable]
        ...
        .. math::
            \vec{l}(θ) =
            R\cos{θ} \hat{x} + R\sin{θ} \hat{y},\quad
            -π ≤ θ ≤ π
        ...
        .. math::
            \hat{B} ≈ \frac{μ_0 I}{4π} \sum_{i=1}^n w_i
            \frac{Δ\vec{l}(π x_i) × \vec{r}(π x_i)}
            {|\vec{r}(π x_i)|^3}
