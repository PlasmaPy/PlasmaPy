...
for element in elements:
    ...
    try:
        ...
        if 'Ionization Energy (eV) (b)' in data.columns:
            data = data.rename(columns={'Ionization Energy (eV) (b)': 'Ionization Energy (eV)'})
        ...
        if 'Ionization Energy (b) (eV)' in data.columns:
            data = data.rename(columns={'Ionization Energy (b) (eV)': 'Ionization Energy (eV)'})
        ...
        data['Sp. Name'] = data['Sp. Name'] + " " + data['Ion Charge'].str.replace('+', '') + "+"
        ...
        data['Ionization Energy (eV)'] = pd.to_numeric(data['Ionization Energy (eV)'], errors='coerce')
        ...
        print(f"Failed to parse data for {element} with exception type {type(e)}: {str(e)}")
