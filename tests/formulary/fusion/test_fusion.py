...
def test_lawson_product_value() -> None:
    r"""Basic arithmetic of n*T*tau."""
    prod = lawson_product(1e20 * u.m**-3, 10 * u.keV, 1 * u.s)
    assert prod.unit.is_equivalent(u.keV * u.s / u.m**3)
    assert np.isclose(prod.value, 1e21)
...
def test_lawson_product_negative_density_raises() -> None:
    r"""Negative density should raise."""
    with pytest.raises((ValueError, u.UnitConversionError)):
        lawson_product(-1e20 * u.m**-3, 10 * u.keV, 1 * u.s)


def test_ignition_condition_self_consistent() -> None:
    r"""When nTtau equals threshold, ratio = 1."""
    T = 30 * u.keV
    tau_E = 1 * u.s
    params = _lookup_reaction("T(d,n)4He")
    sv = reactivity("T(d,n)4He", T)
    E_alpha = params.Q_keV * params.fraction_charged * u.keV
    threshold = 12 * T**2 / (sv * E_alpha)
    n = (threshold / (T * tau_E)).to(u.m**-3)
    ratio = ignition_condition("T(d,n)4He", n, T, tau_E)
    assert_quantity_allclose(ratio, 1.0 * u.dimensionless_unscaled, rtol=1e-10)


def test_ignition_condition_subthreshold() -> None:
    r"""When nTtau is half threshold, ratio should be 0.5."""
    T = 30 * u.keV
    tau_E = 1 * u.s
    params = _lookup_reaction("T(d,n)4He")
    sv = reactivity("T(d,n)4He", T)
    E_alpha = params.Q_keV * params.fraction_charged * u.keV
    threshold = 12 * T**2 / (sv * E_alpha)
    n = (0.5 * threshold / (T * tau_E)).to(u.m**-3)
    ratio = ignition_condition("T(d,n)4He", n, T, tau_E)
    assert_quantity_allclose(ratio, 0.5 * u.dimensionless_unscaled, rtol=1e-10)


def test_ignition_condition_supra() -> None:
    r"""When nTtau is 2x threshold, ratio should be 2."""
    T = 30 * u.keV
    tau_E = 1 * u.s
    params = _lookup_reaction("T(d,n)4He")
    sv = reactivity("T(d,n)4He", T)
    E_alpha = params.Q_keV * params.fraction_charged * u.keV
    threshold = 12 * T**2 / (sv * E_alpha)
    n = (2.0 * threshold / (T * tau_E)).to(u.m**-3)
    ratio = ignition_condition("T(d,n)4He", n, T, tau_E)
    assert_quantity_allclose(ratio, 2.0 * u.dimensionless_unscaled, rtol=1e-10)
...
def test_ignition_condition_invalid_reaction() -> None:
    r"""Invalid reaction should raise."""
    with pytest.raises(ValueError, match="Unknown reaction"):
        ignition_condition("C + O --> Si", 1e20 * u.m**-3, 30 * u.keV, 1 * u.s)


def test_fusion_gain_scales_with_tau() -> None:
    r"""Q should be proportional to tau_E."""
    T = 30 * u.keV
    n = 1e20 * u.m**-3
    Q1 = fusion_gain("T(d,n)4He", T, n, 1 * u.s)
    Q2 = fusion_gain("T(d,n)4He", T, n, 2 * u.s)
    assert np.isfinite(Q1.value)
    assert Q1.value > 0
    assert_quantity_allclose(Q2, 2 * Q1, rtol=1e-10)


def test_fusion_gain_scales_with_density() -> None:
    r"""Q should be proportional to n for two-species fuels."""
    T = 30 * u.keV
    tau = 1 * u.s
    Q1 = fusion_gain("T(d,n)4He", T, 1e20 * u.m**-3, tau)
    Q2 = fusion_gain("T(d,n)4He", T, 2e20 * u.m**-3, tau)
    assert_quantity_allclose(Q2, 2 * Q1, rtol=1e-10)


def test_fusion_gain_scales_with_temperature() -> None:
    r"""Q should peak at intermediate T (near peak reactivity)."""
    n = 1e20 * u.m**-3
    tau = 1 * u.s
    Q10 = fusion_gain("T(d,n)4He", 10 * u.keV, n, tau)
    Q50 = fusion_gain("T(d,n)4He", 50 * u.keV, n, tau)
    Q100 = fusion_gain("T(d,n)4He", 100 * u.keV, n, tau)
    assert Q10.value > 0
    assert Q50.value > Q100.value


def test_fusion_gain_dd_identical_particles() -> None:
    r"""D-D fusion gain should be finite."""
    n = 1e21 * u.m**-3
    tau = 1 * u.s
    Q = fusion_gain("D(d,n)3He", 50 * u.keV, n, tau)
